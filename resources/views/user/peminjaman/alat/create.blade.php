            ...
            <div>
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <div>
                        ...
                        @if ($cart->count() > 0)
                            <a href="{{ route('user.peminjaman.alat.confirm') }}" class="btn btn-info text-white">
                                Konfirmasi
                            </a>
                        @endif
                    </div>
                ...
                <div class="card-body">
                    <div class=" mb-6">
                        <div class="d-flex align-items-center">
                            <div class="">
                                <a href="{{ route('user.peminjaman.alat.peminjaman') }}" class="btn btn-sm btn-primary"><i
                                        class="fas fa-arrow-left me-1"></i>Kembali</a>
                            </div>
                            <div class="ps-3">
                                <h4 class="mb-1">Tabel Alat</h4>
                                <p>Alat yang tersedia periode tanggal {{ $peminjaman['begin_date'] }} hingga
                                    {{ $peminjaman['end_date'] }}</p>
                            </div>
                        </div>
    ...
    @push('scripts')
        ...
        <script>
            ...
            let table = $('#alat-table').DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: "{{ route('user.peminjaman.alat.list', 'alat') }}",
                columns: [{
                        data: 'DT_RowIndex',
                        name: 'DT_RowIndex',
                        orderable: false
            ...
            $('#alat-table').on('click', '.stock', function(e) {
                ...
                (async () => {
                    ...
                    if (text) {
                        $.ajax({
                            url: "{{ url('peminjaman/alat/cart') }}/" + id,
                            type: 'POST',
                            data: {
                                _token: CSRF_TOKEN,
                                qty: text
            ...
            let cartTable = $('#cart-table').DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: "{{ route('user.peminjaman.alat.list', 'cart') }}",
                columns: [{
                        data: 'DT_RowIndex',
                        name: 'DT_RowIndex',
                        orderable: false
