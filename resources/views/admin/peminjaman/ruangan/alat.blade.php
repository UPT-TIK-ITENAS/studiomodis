            ...
            <div>
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <div>
                        ...
                        @if ($cart->count() > 0)
                            <a href="{{ route('admin.peminjaman.ruangan.confirm') }}" class="btn btn-info text-white">
                                Konfirmasi
                            </a>
                        @endif
                    </div>
...
@push('scripts')
    ...
    <script>
        ...
        let table = $('#alat-table').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: "{{ route('admin.peminjaman.ruangan.alat_list', 'alat') }}",
            columns: [{
                    data: 'DT_RowIndex',
                    name: 'DT_RowIndex',
                    orderable: false
        ...
        $('#alat-table').on('click', '.stock', function(e) {
            ...
            (async () => {
                ...
                if (text) {
                    $.ajax({
                        url: "{{ url('admin/peminjaman/ruangan/alat_cart') }}/" + id,
                        type: 'POST',
                        data: {
                            _token: CSRF_TOKEN,
                            qty: text
        ...
        let cartTable = $('#cart-table').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: "{{ route('admin.peminjaman.ruangan.alat_list', 'cart') }}",
            columns: [{
                    data: 'DT_RowIndex',
                    name: 'DT_RowIndex',
                    orderable: false
